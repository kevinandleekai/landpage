...
Vue.component('area-select', {
            ...
            '<div class="area-mask " @click="select" :class="{mask_active: status}"></div>' +
                ...
                '<section class="area-main">'+
                    ...
                    '<ul class="area-data" :class="{none: 1!=areaStatus}">'+
                        '<li class="area-data-item" v-for="(item,key,index) in proviceList" @click="checkProvOne(key,item.name)" :class="{red: key==checkProvince}">'+
    ...
    methods: {
        ...
        //    // axios.get('/landpage/data/test.json').then(function(response){
    ...
    created: function () {
        console.log('proviceList', this.proviceList);
